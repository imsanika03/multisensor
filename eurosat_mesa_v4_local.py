...
class BidirSpectralLayer(nn.Module):
    def __init__(self, dim=768, K=K_REG, d_bidir=D_BIDIR, d_bottle=D_BOTTLE):
        ...
        self.p_gate = nn.Linear(D_SPEC, d_bottle)   # local_feat gate (not spec_ctx)
    ...
    def forward(self, x, spec, local_feat):
        ...
        g = torch.sigmoid(self.p_gate(local_feat))
...
class MESAv4EuroSAT(nn.Module):
    ...
    def forward(self, rgb, x_loc):
        ...
        for i, blk in enumerate(self.dino.blocks[N_ADAPT_START:]):
            x = blk(x)
            x, spec = self.bidir_layers[i](x, spec, local_feat)
        x   = self.dino.norm(x)
        cls = x[:, 0]
        z   = x[:, 1:].mean(1)
...
def main():
    ...
    json.dump(results, open("results/eurosat_mesa_v4_local.json", "w"), indent=2)

    print("\n--- SUMMARY ---", flush=True)
    print(f"  DINOv2 LP (baseline)                : 96.78%", flush=True)
    print(f"  MESA-v4-local EuroSAT ({len(SEEDS)} seeds)   : "
